...
const AIAssistant = () => {
  ...
  const exampleQuestions = [t.aiExampleQ1, t.aiExampleQ2, t.aiExampleQ3, t.aiExampleQ4];
  ...
  const streamFromEdgeFunction = async (allMessages: Message[]) => {
    ...
    if (remaining !== null) setRemainingRequests(parseInt(remaining, 10));

    if (!resp.ok) {
      const data = await resp.json().catch(() => ({}));
      if (resp.status === 429) {
        const msg = data?.error || `Daily limit of ${MAX_DAILY} messages reached.`;
        toast.error(msg);
        throw new Error(msg);
      }
      if (resp.status === 402) {
      ...
      <header className="bg-primary text-primary-foreground p-6 shadow-md">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 text-primary-foreground/80 text-xs">
            <Wifi size={14} />
            <span>{t.aiConnectionStatus}</span>
          </div>
        </div>
        {remainingRequests !== null && (
          <div className="mt-2 text-primary-foreground/70 text-xs">
            {remainingRequests} / {MAX_DAILY} {t.aiDailyRemaining}
          </div>
        )}
      </header>

      <div className="bg-medical-yellow/10 border-l-4 rtl:border-l-0 rtl:border-r-4 border-medical-yellow p-4 m-4 rounded-r-lg rtl:rounded-r-none rtl:rounded-l-lg">
        <div className="flex gap-2">
          <AlertCircle className="text-medical-yellow flex-shrink-0 mt-0.5" size={20} />
          <p className="text-sm text-foreground">{t.aiDisclaimer}</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 ? (
          <div className="text-center py-8">
            ...
            <h2 className="text-xl font-bold text-foreground mb-2">{t.aiWelcomeTitle}</h2>
            <p className="text-muted-foreground mb-6 px-4">{t.aiWelcomeDesc}</p>
            <div className="space-y-2">
              <p className="text-sm font-semibold text-foreground mb-3">{t.aiExampleQuestionsTitle}</p>
